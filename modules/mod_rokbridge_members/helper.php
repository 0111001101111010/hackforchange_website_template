...
class modRokBridgeMembersHelper
{
	...
	function modRokBridgeMembersHelper(&$rokbridge) {
		...
		if (!class_exists('RokBridgeHelper')) {
		    $app->enqueueMessage(JText::_('Error initializing mod_rokbridge_members, RokBridge not installed.'),"error");
		    return;
	...
	function getList($params )
	{
		...
		$avatar_size = $params->get('avatar_size',32);		
	
		foreach ($rows as $row) {
		    if ($modulemode=='latest') {
    		    $date = JFactory::getDate($row->reg_date);
        		$reg_date = $date->format($formatdate);
    	    ...
    	    $row->avatar_img = $rokbridge->getAvatar($row,$avatar_size,$row->extra_info,$default_avatar);
		    $latest_members[] = $row;
		}
