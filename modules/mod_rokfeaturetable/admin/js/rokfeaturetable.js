...
document.id("no_of_rows").set("text",Tabs.wrappers.length);document.id("no_of_columns").set("text",Panels.rows.length);},parse:function(d){var b=d;var a=b.split("---"),c=[],e={};
if(!a.length){return;}a=a.slice(0,6);a.each(function(h,g){h=h.replace(/\n{1,}/g,"\n");var f=h.split("\n");if(f.length){f=f.erase("");e={};f.each(function(i,k){var m=i.split("::");
e[m[0]]={value:m[1]};var l=m[1].split("|");if(l[1]){e[m[0]]={value:l[1],style:l[0]};}["-sub","-link","-classes"].each(function(o){var j=o.replace("-","");
var n=m[0].replace(o,"");if(e[n]&&n!=m[0]){e[n][j]=m[1];delete e[m[0]];}});});c.push(e);}});return c;},importTemplate:function(c){RFT.clean();var a=RFT.parse(c),b=0;
a.each(function(e,g){if(g>0){Tabs.plus.fireEvent("click");}var f=0;for(var h in e){if(f>0){Panels.addRow(Panels.wrappers[g]);}var d=Panels.rows[b].getElements("input[type=text]");
d[0].set("value",e[h]["value"]);d[1].set("value",h+(e[h]["classes"]?" "+e[h]["classes"]:""));if(e[h]["sub"]){d[2].set("value",e[h]["sub"]);}if(e[h]["link"]){d[3].set("value",e[h]["link"]);
