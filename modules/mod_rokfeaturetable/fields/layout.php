<?php
/**
 * @version   $Id: layout.php 6825 2013-01-28 06:18:42Z btowles $
...
class JFormFieldLayout extends JFormField {
    ...
    public function getInput()
	{
		...
		$this->data = $this->getData($this->element);
		...
		$render .= "		Current Table Layout: <span id='no_of_rows'>".$this->element['rows']."</span> x <span id='no_of_columns'>".$this->element['columns']."</span>\n";
		$render .= "	</div>\n";
	...
	for ($i = 0; $i < $this->counter; $i++) {
		...
		foreach($this->data as $data) {
			$render .= "	<textarea id='jform_params_".$data['name']."' name='jform[params][".$data['name']."]' class='text_area' rows='10' cols='50'>".$data['value']."</textarea>\n";
		}
		$render .= "</div>\n";
     ...
     */function getData($node) {
		...
		foreach($node->children() as $child) {
			...
			$value = $this->form->getValue($name, 'params');
     ...
     */function createRow($data, $j) {
			...
			$value = $this->getValue($values, 'params');
