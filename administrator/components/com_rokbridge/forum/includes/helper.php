...
class JForumHelper
{
	public static function loadPHPBB3($path) 
	{
		...
		if(!defined('STRIP')) {
			define('STRIP', (@ini_get('magic_quotes_gpc')=='1') ? true : false);
		} 
		...
		require_once( JPATH_FORUM.'/configuration.php' );
		$config = new JConfigForum();
	
		$phpbb_root_path = JPATH_ROOT.'/'.$config->phpbb_path.'/';
		...
		require_once(JPATH_FORUM.'/includes/utf8.php');
